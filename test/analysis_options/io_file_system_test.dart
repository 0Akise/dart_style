...
void main() {
    ...
    group('parentDirectory()', () {
        ...
        Future<String?> parent(String path) async => (await fs.parentDirectory(await fs.makePath(path))).ioPath;
    ...
    group('readFile()', () {
        test('reads a file', () async {
            ...
            var fs = IOFileSystem();
            expect(await fs.readFile(await fs.makePath(p.join(d.sandbox, 'dir', 'some_file.txt'))), 'contents');
            expect(await fs.readFile(await fs.makePath(p.join(d.sandbox, 'dir', 'sub', 'another.txt'))), 'more');
        });
