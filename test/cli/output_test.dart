...
void main() {
    ...
    group('--show', () {
        ...
        test('changed shows changed files', () async {
            ...
            await expectLater(
                process.stdout,
                emitsInOrder(['Formatted ${p.join('code', 'a.dart')}', 'Formatted ${p.join('code', 'c.dart')}']),
            );
            await expectLater(process.stdout, emits(startsWith('Formatted 3 files (2 changed)')));
            await process.shouldExit(0);
        });
    ...
    group('--output', () {
        group('show', () {
            test('prints only formatted output by default', () async {
                await d.dir('code', [d.file('a.dart', unformattedSource), d.file('b.dart', formattedSource)]).create();
        ...
        group('none', () {
            test('with --show=all prints only names', () async {
                ...
                await expectLater(
                    process.stdout,
                    emitsInOrder(['Changed ${p.join('code', 'a.dart')}', 'Unchanged ${p.join('code', 'b.dart')}']),
                );
                await expectLater(process.stdout, emits(startsWith('Formatted 2 files (1 changed)')));
                await process.shouldExit(0);
    ...
    group('--summary', () {
        test('line', () async {
            ...
            await expectLater(process.stdout, emits(matches(r'Formatted 2 files \(1 changed\) in \d+\.\d+ seconds.')));
            await process.shouldExit(0);
        });
    });
}
