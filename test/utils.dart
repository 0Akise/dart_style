...
Future<TestProcess> runFormatter([List<String>? args]) {
    ...
    return TestProcess.start(Platform.resolvedExecutable, [_formatterPath!, ...?args], workingDirectory: d.sandbox);
}
        ...
        for (var formatTest in testFile.tests) {
            ...
            for (var output in formatTest.outputs) {
                ...
                if (output.version case var version?) {
                    ...
                    _runTestAtVersion(testFile, formatTest, output, DartFormatter.latestShortStyleLanguageVersion);
                }
            }
        }
    });
...
SourceCode _validateFormat(
    ...
    } else if (actual.selectionStart != expected.selectionStart || actual.selectionLength != expected.selectionLength) {
        fail(
...
/// for each package.
d.DirectoryDescriptor packageConfig(String rootPackageName, {String? version, Map<String, String>? packages}) {
